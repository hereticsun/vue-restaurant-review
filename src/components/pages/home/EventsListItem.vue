    ...
    <div class="events-list__event-content">
      <div class="events-list__event--front" style="background-size: cover;" :style="backgroundImage(event.image)">
        <header class="events-list__event-header">
          <h3>{{ event.venue }}</h3>
          <dl>
            <dt>Average Rating:</dt><dd>{{ averageRating(event.rating) }}</dd>
            <dt>Chosen by:</dt><dd>{{ event.bananaMeister }} &ndash; {{ formattedDate(event.date) }}</dd>
          </dl>
        </header>
      </div>
      <div class="events-list__event--back">
        ...
        <ul class="event-info">
          <li class="event-info__venue"><h4>{{ event.venue }}</h4></li>
